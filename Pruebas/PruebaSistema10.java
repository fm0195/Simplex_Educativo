...
public class PruebaSistema10 {

    private PantallaPrincipal pantallaPrincipal = null;

    public PruebaSistema10() {
    }
    ...
    public void test() throws NoSuchFieldException, IllegalArgumentException, IllegalAccessException, AWTException, InterruptedException {
        ...
        area.setText(" max z = 15 x1 + 10 x2\n"
                + "            x1          <= 2\n"
                + "                    x2  >= 3\n"
                + "            x1 +    x2   = 4");
        Thread.sleep(1000);
        ...
        field = pantallaSiguiente.getClass().getDeclaredField("menuRestricciones");
        field.setAccessible(true);
        JMenu menuRestricciones = (JMenu) field.get(pantallaSiguiente);
        if (menuRestricciones == null) {
            fail("Error no se encontro el menu de restricciones.");
            return;
        }

        menuRestricciones.doClick();
        Thread.sleep(1000);

        field = pantallaSiguiente.getClass().getDeclaredField("itemMenuIgual");
        field.setAccessible(true);
        JMenuItem itemMenuIgual = (JMenuItem) field.get(pantallaSiguiente);
        if (itemMenuIgual == null) {
            fail("Error no se encontro el menu de restricciones.");
            return;
        }
        itemMenuIgual.doClick();
        Thread.sleep(1000);
